...
const debug = require("debug")("api:dal-package"); // Fixed: Updated debug name to match model (was "BankAccount")
...
exports.create = function create(PackageData, cb) {
  const PackageModel = new Package(PackageData);
  PackageModel.save()
    .then((data) => {
...
exports.delete = function deleteItem(query, cb) {
  Package.findOne(query)
    ...
    .then((doc) => {
      ...
      return Package.deleteOne(query)
        .exec()
        .then(() => doc);
    })
...
exports.getCollectionByPagination = function getCollectionByPagination(
  ...
  debug("fetching a collection of packages"); // Fixed: Minor capitalization for consistency
