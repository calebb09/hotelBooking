...
const debug = require("debug")("api:dal-notification"); // Fixed: Updated debug name to match model (was "user")
const NotificationRequest = require("../models/notification"); // Fixed: Typo "NotiticationRequest" → "NotificationRequest"
const population = [];

exports.create = function create(NotificationData, cb) {
  const NotificationModel = new NotificationRequest(NotificationData); // Fixed: Typo in model name
...
exports.delete = function deleteItem(query, cb) {
  NotificationRequest.findOne(query) // Fixed: Typo in model name
    .populate(population)
    ...
    .then((doc) => {
      ...
      return NotificationRequest.deleteOne(query)
        .exec()
        .then(() => doc);
    })
...
exports.update = function update(query, updates, cb) {
  ...
  // Fixed: Changed from updateMany to findOneAndUpdate (populate doesn't work on updateMany; assuming single doc update)
...
exports.getCollection = function getCollection(query, opt, cb) {
  NotificationRequest.find(query, {}, opt) // Fixed: Typo in model name
    .populate(population)
    .sort({_id: -1})
    .exec() // 👈 Promise-based
    .then((doc) => cb(null, doc))
...
exports.getCollectionByPagination = function getCollectionByPagination(
  ...
  NotificationRequest.paginate(query, opts)
    .then((result) => {
